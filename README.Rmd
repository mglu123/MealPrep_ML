...
## Functions
`find_fruits_veg()`: This function will drop rows with NAs and find the index of columns with all
...
To find the outliers in the dataframe, run `mealprep.find_bad_apples(df)` and you'll get a dataframe that shows which columns have outliers, which rows they can be found in, and how many outliers are in those columns.
...
find_fruits_veg() will help you find the column indices for either numerical or categorical variables in your data frame. 
The example below shows how to use find_fruits_veg() to find the categorical columns' index of a toy data frame.
...
Then, apply the find_fruits_veg() function to the data frame.
```python
