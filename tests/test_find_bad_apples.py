...
def test_find_bad_apples():
    '''
    This function tests the following behaviour for
    the find_bad_apples() function:
    1. That find_bad_apples() raises an exception
        when a dataframe with non-numeric columns
        is passed as an argument.
    2. That find_bad_apples() raises an exception
        when a dataframe with fewer than 30 rows
        is passed as an argument.
    3. That find_bad_apples() produces the correct output
        when a dataframe with positive and negative values
        in the same column is passed as an argument.
    4. That find_bad_apples() returns
        'Congratulations! You have no bad apples.'
        when there are no outliers present in the dataframe.
    5. That find_bad_apples() produces the correct output
        when a dataframe with only one column
        is passed as an argument.
    6. That find_bad_apples() produces the correct output
